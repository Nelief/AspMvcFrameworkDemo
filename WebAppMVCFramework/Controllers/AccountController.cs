    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ////Temp: Role Creation/Assignment
                    ////creo 2 istanze per roleStore e roleManager, lo store sarà collegato al UserDB esistente, il manager sarà usato per configurare il nuovo utente
                    //var roleStore = new RoleStore<IdentityRole>(new ApplicationDbContext());
                    //var roleManager = new RoleManager<IdentityRole>(roleStore);

                    ////definisco il ruolo
                    //string roleName = "canManageMovies";

                    ////Creo il nuovo ruolo, questo sarà di tipo "IdentityRole" e nominato in base alla stringa passata in input 
                    //await roleManager.CreateAsync(new IdentityRole(roleName));

                    ////Associo l' utente al ruolo appena creato 
